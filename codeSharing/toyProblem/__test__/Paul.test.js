...
const test = (i) => {
	...
	if (i === 2) {
		const func = require('../02_nonrepeatCharacter/Paul');
	...
	if (i === 3) {
		const func = require('../03_isSubsetOf/Paul');
	...
	if (i === 4) {
		const func = require('../04_nthFibonacci/Paul');
	...
	if (i === 5) {
		const func = require('../05_bubbleSort/Paul');
	...
	if (i === 6) {
		const func = require('../06_commonCharacters/Paul');
	...
	if (i === 7) {
		const Tree = require('../07_treeDFSelect/Paul');
	...
	if (i === 8) {
		const func = require('../08_largestProductOfThree/Paul');
	...
	if (i === 9) {
		const func = require('../09_deepEquality/Paul');
	...
	if (i === 10) {
		const func = require('../10_rotatedArraySearch/Paul');
	...
	if (i === 11) {
		const func = require('../11_powerSet/Paul');
